%%%读取地理坐标信息%%%
[noUes,range] = geotiffread('D:\BiyeData\sentinel1-PDtif-LuoTu\0614VV.tif'); %%range为地理信息
%%%读取地理坐标信息%%%

%%%%文件夹%%%%
% filePath = 'D:\BiyeData\sentinel1-PDtif-LuoTu';
filePath = 'D:\BiyeData\sentinel1-PDtif';
dirFile = dir(filePath);
[DataNum,noUse] = size(dirFile);
DataLength = (DataNum-2)  / 4; %获取数据的总数
%%%%文件夹%%%%

%首先读取VH数据，建立一个cell来存储VH数据
VH_tif = cell(DataLength,1);
for i = 1:4:117
    fileName = dirFile(i+2,1).name;
    PathName = [filePath,'\',fileName];
    VH_tif{(i+3)/4 , 1} = imread(PathName);
end
...
for i = 3:4:119
    ...
    RiQi{(i+1)/4  , 1} = fileName(1:4);
    
    PathName = [filePath,'\',fileName];
    VV_tif{(i+1)/4 , 1} = imread(PathName);
end
