...
%image = xlsread('E:\研二 下学期\6-14农科村实地测量\农科村数据 激光数据\19-06-14-2526\19-06-14-2526_11.csv');


% image = xlsread('E:\研二 下学期\6-14农科村实地测量\农科村数据 激光数据\19-06-14-0102\19-06-14-0102_2.csv');%%%点1
% image = xlsread('E:\研二 下学期\6-14农科村实地测量\农科村数据 激光数据\19-06-14-0304\19-06-14-0304_3.csv'); %%%点2
% image = xlsread('E:\研二 下学期\6-14农科村实地测量\农科村数据 激光数据\19-06-14-0506\19-06-14-0506_2.csv'); %%%点3
% image = xlsread('E:\研二 下学期\6-14农科村实地测量\农科村数据 激光数据\19-06-14-0708\19-06-14-0708_2.csv');  %点四
% image = xlsread('E:\研二 下学期\6-14农科村实地测量\农科村数据 激光数据\19-06-14-1314\19-06-14-1314.csv');  %点7
% image = xlsread('E:\研二 下学期\6-14农科村实地测量\农科村数据 激光数据\19-06-14-15-16\19-06-14-15-16_9.csv');  %点8
%  image = xlsread('E:\研二 下学期\6-14农科村实地测量\农科村数据 激光数据\19-06-14-1718\19-06-14-1718_2.csv');  %点9
% imageLength1 = 730806;               %%%%%当长度超过时用这个
imageLength1 = image(1,1);
%image = image(1:imageLength1 + 1 , :);    %%%%%当长度超过时用这个
...
%我们先设定每条线都分200个点出来，以y轴为分割线进行，先用一个矩阵存储在y轴上的间隔距离
...
for i = 1:1:StripeSize
    EachLength(i,1) = abs(Stripe(1,2,i) - Stripe(LineSize,2,i)); %%计算每条线段在Y分量上的的长度
...
for i = 1:1:StripeSize
    ...
    %构造Cut一维矩阵
    for j = 1:1:200
        CutLine(j,1) = Stripe(1,2,i) - (j-1) * spaceOfEach(i,1);       %%%%%%%%%%%%%%由是增是减来决定%%%%%%%%%%%%%%%
    end
    %构造Cut一维矩阵
    
    %寻找离每个间隔处最近的点的坐标
    for j = 1:1:200
        [minNum , SamplePointLoc(j,1) ] = min(abs(Stripe(:,2,i) - CutLine(j,1))); 
    end
    %寻找离每个间隔处最近的点的坐标
